...
- `OMBS.OB.AT` turns on "assessment standing in the context," which is illustrated (teacher, coach, assessor, reviewer) but not defined. An adopter could declare that a parent has assessment standing and print attested band claims while remaining technically conformant — reintroducing the inflation `OMBS.EV.WARR` exists to prevent. Requiring that standing be conferred and recorded is a candidate amendment for the comment window.
